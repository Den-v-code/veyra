"""Fresh guarded Lean stage/object compilation for the R12.5 formal bridge."""
...
from .intrinsic_vam_formal_bridge_io import _clean_env, _runtime_identity
from .intrinsic_vam_formal_manifest import EXPECTED_LEAN_RUNTIME
from .intrinsic_vam_formal_objects import (
    _EXPECTED_R12_5_OBJECT_ROWS,
    EXPECTED_R12_5_OBJECTS,
)
from .intrinsic_vam_formal_report import valid_object_manifest
from .intrinsic_vam_formal_snapshot import (
    _SNAPSHOT_NAME_ROWS,
    IntrinsicVamLeanSnapshot,
    verify_intrinsic_vam_snapshot,
)
...
_GUARDED_DOMAIN = b"veyra-r12.5-guarded-input-v1\0"
...
def _reviewed_closure(
    ...
    logger.debug("intrinsic_vam_formal_compile._reviewed_closure entry label=%s", label)
    ...
    logger.debug("intrinsic_vam_formal_compile._reviewed_closure exit")
    return result


def _source_closure(
    snapshot: IntrinsicVamLeanSnapshot,
    sources: Mapping[str, bytes],
) -> ProtectedClosure:
    logger.debug("intrinsic_vam_formal_compile._source_closure entry")
    records = tuple(
        (snapshot.root / filename, len(sources[name]), sha256(sources[name]).digest())
        for name, filename in _SNAPSHOT_NAME_ROWS
    )
    result = _reviewed_closure("r12.5-snapshot-source", snapshot.root, records)
    logger.debug("intrinsic_vam_formal_compile._source_closure exit")
    return result
...
def _object_closure(
    ...
    logger.debug("intrinsic_vam_formal_compile._object_closure entry count=%d", len(objects))
    reviewed = dict(_EXPECTED_R12_5_OBJECT_ROWS)
    records = tuple(
        (path, reviewed[name][1], bytes.fromhex(reviewed[name][2]))
        for name, path in objects
    )
    result = _reviewed_closure("r12.5-prior-object", run_root, records)
    logger.debug("intrinsic_vam_formal_compile._object_closure exit")
    return result


def _validate_fresh_object(run_root: Path, name: str, path: Path) -> None:
    logger.debug("intrinsic_vam_formal_compile._validate_fresh_object entry name=%s", name)
    object_names = tuple(name for name, _ in _SNAPSHOT_NAME_ROWS[:-1])
    if not valid_object_manifest(EXPECTED_R12_5_OBJECTS, object_names):
        raise ValueError("r12.5-lean-object-manifest-shape-mismatch")
    filename, size, digest = dict(_EXPECTED_R12_5_OBJECT_ROWS)[name]
    try:
        entries, metadata = tuple(path.parent.iterdir()), path.lstat()
    except OSError as exc:
        raise ValueError("r12.5-lean-object-unreadable") from exc
    if (
        path.name != filename
        or entries != (path,)
        ...
        raise ValueError("r12.5-lean-object-shape-mismatch")
    ...
    if paths_digest((path,), run_root, _GUARDED_DOMAIN) != expected:
        raise ValueError("r12.5-lean-object-digest-mismatch")
    logger.debug("intrinsic_vam_formal_compile._validate_fresh_object exit")


def _runtime_absences() -> tuple[Path, ...]:
    logger.debug("intrinsic_vam_formal_compile._runtime_absences entry")
    ...
    result = tuple(lean_library / row[0] for _, row in _EXPECTED_R12_5_OBJECT_ROWS) + (
    ...
    logger.debug("intrinsic_vam_formal_compile._runtime_absences exit count=%d", len(result))
    return result
...
def compile_snapshot(
    ...
    """Fresh-compile all stages under runtime/source/prior-object guards."""
    logger.debug("intrinsic_vam_formal_compile.compile_snapshot entry stages=%d", STAGES)
    ...
    try:
        with (snapshot.root.parent / ".materialize.lock").open("a+b") as lock:
            ...
            _runtime_identity()
            verify_intrinsic_vam_snapshot(snapshot, lean_sources)
            source_closure = _source_closure(snapshot, lean_sources)
            names = tuple(name for name, _ in _SNAPSHOT_NAME_ROWS[:-1])
            if not valid_object_manifest(EXPECTED_R12_5_OBJECTS, names):
                raise ValueError("r12.5-lean-object-manifest-shape-mismatch")
        ...
        logger.error("intrinsic VAM formal compile blocked=%s", exc)
        return False, str(exc)
    result = ";".join(diagnostics)
    logger.debug("intrinsic_vam_formal_compile.compile_snapshot exit")
    return True, result
