...
- `tests/test_veyra_sage_notebooks.py` — summary, markdown, ipynb shape, artifact writes, invalid-cell failure.
- `tests/test_veyra_sage_notebook_artifacts.py` — current notebook inventory and real disk bundle writes.
- `tests/test_veyra_sage.py` — certificate suite includes `sage_notebook_passed`.
