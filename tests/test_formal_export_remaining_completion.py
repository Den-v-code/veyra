...
def test_existing_and_final_rows_share_actual_rebound_artifact_hashes():
    ...
    assert hashlib.sha256(Path("proofs/lean/VeyraAlgebra.lean").read_bytes()).hexdigest() == ANALYSIS_ARTIFACT_SHA256
    assert hashlib.sha256(Path("proofs/lean/VeyraCyclic.lean").read_bytes()).hexdigest() == CYCLIC_ARTIFACT_SHA256
