"""Exact replay bridge from current one-shot Native Formation v3 into Q_sem."""
...
CONTRACT_VERSION = "p3og-semantic-formation-bridge-contract-v2"
EVIDENCE_VERSION = "p3og-semantic-formation-bridge-evidence-v2"
BRIDGE_RULE_ID = "fresh-one-shot-native-formation-replay-equals-q-sem-tick-v2"
CLOSURE_RULE_ID = "genuine-departure-then-first-q-sem-seed-return-v2"
...
def p3og_semantic_formation_bridge_contract(
    ...
    """Commit the selection-free current relation between formation and Q_sem."""
...
def validate_semantic_formation_bridge_contract(
    ...
    """Freshly reconstruct the complete selection-free current bridge contract."""
...
def build_p3og_semantic_formation_bridge_evidence(
    ...
    """Replay one witnessed current formation and prove its genealogy lives in Q_sem."""
    (
        source,
        autonomous_source,
        semantic_contract,
    ...
    if (
        ...
        or formation_evidence.initial_state.boundary
        is not NativeFormationBoundary.UNFORMED
        or formation_evidence.final_state.boundary
        is not NativeFormationBoundary.ALIVE
    ):
    ...
    if (
        seed.seed_digest != selection.selected_seed_digest
        or seed.seed_digest != formation_source.selected_seed_digest
    ):
    ...
    for index, expected_formation_tick in enumerate(
        ...
        if closed_after and index != len(formation_evidence.ticks):
            raise ValueError(
                "p3og-semantic-formation-bridge-not-first-terminal-closure",
            )
    ...
    fields = (
        ...
        "current-one-shot-native-formation-is-first-return-in-q-sem",
        genealogy,
        0,
        P3OG_SEMANTIC_FORMATION_BRIDGE_NONCLAIMS,
    )
...
def _preflight_bridge_evidence(
    ...
    try:
        ...
        bridge_contract_digest = evidence.bridge_contract_digest
        formation_source_digest = evidence.formation_source_digest
        formation_evidence_digest = evidence.formation_evidence_digest
        selection_source_digest = evidence.selection_source_digest
        selection_receipt_digest = evidence.selection_receipt_digest
        selected_seed_digest = evidence.selected_seed_digest
    ...
    if (
        ...
        or len(steps) > MAX_FORMATION_TICKS
...
def validate_p3og_semantic_formation_bridge_evidence(
    ...
    """Freshly rebuild the exact current bridge and reject any replay drift."""
