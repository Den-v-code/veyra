...
Implemented in `src/core/theorem_registry.py`:
...
Tests in `tests/test_theorem_registry.py` verify registry summary, dependency edges, ready Pythagorean card, missing-dependency blocking, card obstruction blocking, and tangent line-shell card readiness.
