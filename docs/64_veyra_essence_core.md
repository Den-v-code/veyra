...
**Implementation:** `src/core/essence.py`, `veyra_sage/essence.py`; R9 `intrinsic_mode_*`; R10 `proof_surface_*` and `proof_elaboration_*`; R11 `observer_core_*`; R13 `intrinsic_observer_echo_*`.
...
ontological postulates. `149_positive_ontology_p0.md` is the canonical
...
```python
from src.core.essence import essence_report
...
```bash
PYTHONDONTWRITEBYTECODE=1 python3 -m pytest -q tests/test_essence_core.py tests/test_veyra_sage_essence.py tests/test_certify.py tests/test_veyra_sage.py
