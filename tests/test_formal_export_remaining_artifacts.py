...
def test_remaining_metadata_pins_actual_whole_file_digests():
    ...
    algebra = Path("proofs/lean/VeyraAlgebra.lean").read_bytes()
    cyclic = Path("proofs/lean/VeyraCyclic.lean").read_bytes()
...
def test_analysis_artifact_states_only_the_three_fixed_fixtures():
    ...
    text = Path("proofs/lean/VeyraAlgebra.lean").read_text()
    literals = (
...
def test_cyclic_artifact_preserves_c001_and_pins_fixed_chord_mirror():
    ...
    text = Path("proofs/lean/VeyraCyclic.lean").read_text()
...
def test_new_declarations_are_closed_and_have_no_general_quantifiers():
    ...
    texts = {
        **{symbol: Path("proofs/lean/VeyraAlgebra.lean").read_text() for symbol in SYMBOLS[:3]},
        SYMBOLS[3]: Path("proofs/lean/VeyraCyclic.lean").read_text(),
    }
