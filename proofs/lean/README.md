...
| `VeyraDoctrinalInduction.lean` | DI-1 candidate shadow laws: pinned classical shadow of the doctrinal license, replay base/step laws, exact replay count, one-block extension | `FORMALLY_PROVED` | `THM_DI1_001_family_from_base_and_step`, `THM_DI1_002_replay_zero`, `THM_DI1_003_replay_succ`, `THM_DI1_004_replay_count`, `THM_DI1_005_block_extension_shadow` |
...
| `VeyraRealizationTransport.lean` | abstract inverse-image partition identity/composition, label-partition pullback, raw off-diagonal distinction, accepted-row zero-residual bridge, bottom/refinement preservation, and conditional cost nonincrease | `INTERNAL_RESEARCH_CANDIDATE` | none; exhaustive Python small-map rows pressure normalization, production `distinction_set`, and production `observer_descent`, where every accepted target closure row descends to the receipt-selected source observer with empty residual; no Python implementation equivalence, general R16 descent totality, residual-chain correspondence, P1-A vertical law, category/functor/naturality, or cross-doctrine theorem |
