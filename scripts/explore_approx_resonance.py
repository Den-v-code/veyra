#!/usr/bin/env python3
"""Explore bounded-defect approximate Veyra resonance with progress."""
...
from src.core.approx_resonance import approximate_resonance_profile  # noqa: E402
from src.core.modes import Mode, enumerate_modes  # noqa: E402

logger = logging.getLogger("veyra.explore_approx_resonance")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Explore Veyra approximate resonance.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"part={part.word} candidates={len(wholes)} max_defects={args.max_defects}")

        stage(2, 3, "Profiling bounded-defect resonance")
        shown = 0
        for whole in wholes:
            profile = approximate_resonance_profile(part, whole, args.max_defects)
            if profile.resonates or profile.obstruction in {"over-budget", "bounded-defect"}:
                best = profile.best
                defects = () if best is None else tuple((d.index, d.expected, d.actual) for d in best.defects)
                offset = None if best is None else best.offset
                count = None if best is None else best.defect_count
                print(
                    f"whole={whole.word} resonates={profile.resonates} obstruction={profile.obstruction} "
                    f"best_offset={offset} defects={count} detail={defects}"
                )
                shown += 1
                if shown >= args.limit:
                    break
