#!/usr/bin/env python3
"""Explore weighted tact-specific Veyra resonance with progress."""
...
from src.core.weighted_resonance import CostMap, weighted_resonance_profile  # noqa: E402

logger = logging.getLogger("veyra.explore_weighted_resonance")
...
def parse_costs(raw: str) -> CostMap:
    """Parse costs like b>c:0.25,a>c:2."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Explore weighted Veyra resonance.")
    parser.add_argument("--alphabet", default="abc", help="whole alphabet, default: abc")
    ...
    parser.add_argument("--budget", type=float, default=0.5, help="weighted defect budget, default: 0.5")
    parser.add_argument("--costs", default="b>c:0.25", help="directed costs, e.g. b>c:0.25,a>c:2")
    parser.add_argument("--default-cost", type=float, default=1.0, help="default mismatch cost")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"part={part.word} candidates={len(wholes)} budget={args.budget} costs={costs}")

        stage(2, 3, "Profiling weighted resonance")
        shown = 0
        for whole in wholes:
            profile = weighted_resonance_profile(part, whole, args.budget, costs, args.default_cost)
            if profile.resonates or profile.obstruction in {"weighted-defect", "over-budget"}:
                best = profile.best
                detail = () if best is None else tuple((d.index, d.expected, d.actual, d.cost) for d in best.defects)
                offset = None if best is None else best.offset
                total = None if best is None else best.total_cost
                print(
                    f"whole={whole.word} resonates={profile.resonates} obstruction={profile.obstruction} "
                    f"offset={offset} cost={total} detail={detail}"
                )
                shown += 1
                if shown >= args.limit:
                    break
