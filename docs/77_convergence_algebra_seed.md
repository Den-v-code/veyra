...
Implemented in `src/core/convergence_algebra.py`:
...
`src/core/certify_convergence.py` adds `convergence_algebra` to the executable certificate suite.
...
```bash
PYTHONPATH=. python3 -m pytest -q tests/test_convergence_algebra.py tests/test_certify.py tests/test_essence_core.py
