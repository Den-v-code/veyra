...
**Implementation:** `veyra_sage/geometry_cards.py`, `src/core/geometry_visual_regression.py`.
...
`VeyraGeometryTheoremLab` is public through `veyra_sage.all` for current notebooks and tests, but remains under the research-lab boundary from `docs/69_package_boundary.md`.
...
```bash
PYTHONPATH=. python3 -m pytest -q tests/test_veyra_sage_geometry_cards.py tests/test_veyra_sage_notebook_artifacts.py tests/test_veyra_sage.py tests/test_veyra_sage_api_index.py
