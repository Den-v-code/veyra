#!/usr/bin/env python3
"""Explore schema compatibility and Veyra prime variants with progress."""
...
from src.core.modes import Mode, enumerate_modes, substitute_mode  # noqa: E402
from src.core.primes import prime_profile  # noqa: E402

logger = logging.getLogger("veyra.explore_prime_variants")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Explore Veyra schema compatibility and prime variants.")
    parser.add_argument("--alphabet", default="ab", help="mode alphabet, default: ab")
    parser.add_argument("--max-len", type=int, default=4, help="maximum mode length, default: 4")
    parser.add_argument("--mapping", default="a:x,b:yy", help="substitution map, default: a:x,b:yy")
    parser.add_argument("--limit", type=int, default=12, help="row limit, default: 12")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        stage(1, 4, "Enumerating multi-tact modes")
        ...
        stage(2, 4, "Checking substitution schema compatibility")
        mapping = parse_mapping(args.mapping)
        schema = lambda mode: substitute_mode(mode, mapping)
        respects = unary_respects(modes, schema, "length", "length", "substitute")
        failures = unary_compatibility_failures(modes, schema, "length", "length", "substitute", args.limit)
        print(f"substitute respects (T_len,T_len): {respects}")
        for failure in failures[: args.limit]:
            print(failure)

        stage(3, 4, "Profiling prime variants")
        for mode in modes[: args.limit]:
            profile = prime_profile(mode, tact=alphabet[0])
            print(
                f"{mode.word}: numeric={profile.numeric_prime} "
                f"ordered={profile.ordered_primitive} cyclic={profile.cyclic_primitive} "
                f"resonance={profile.ordered_resonance_prime}"
            )

        stage(4, 4, "Done")
        print(f"[done] processed={len(modes)} failures={len(failures)} errors=0")
