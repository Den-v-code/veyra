...
- `tests/test_surprise.py` — hidden edit-lift witness, no-surprise negative case, checklist.
- `tests/test_surprise_separation.py` — first finite baseline-blind separation row and no-overclaim boundary.
- `tests/test_surprise_search.py` — bounded expanded-baseline search ledger and XOR/parity hidden-correlation row.
- `tests/test_surprise_kwise.py` — S5 k-wise/global-parity hidden-correlation row.
- `tests/test_surprise_debruijn.py` — S6 order-3-window-blind de Bruijn trail row.
