...
| `VeyraRealizationTransport.lean` | abstract inverse-image partition identity/composition, label-partition representation/pullback bridge, bottom/refinement preservation, and conditional cost nonincrease | `INTERNAL_RESEARCH_CANDIDATE` | none; relation-level helpers plus an explicit `SamePartitionLabels` normalization premise; exhaustive Python small-map rows pressure that premise, but no Python implementation equivalence, R16 descent existence, P1-A vertical law, category/functor/naturality, or cross-doctrine theorem |
