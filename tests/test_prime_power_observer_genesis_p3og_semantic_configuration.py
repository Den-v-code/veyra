...
def test_contract_is_selection_free_and_commits_exact_finite_operations() -> None:
    ...
    assert "standalone-semantic-configuration-is-not-history-evidence" in (
        P3OG_SEMANTIC_CONFIGURATION_NONCLAIMS
    )
    assert "full-def-og-009-discharge" in P3OG_SEMANTIC_CONFIGURATION_NONCLAIMS
    assert "endogenous-observer-role" in P3OG_SEMANTIC_CONFIGURATION_NONCLAIMS
    assert "historical-actualization" in P3OG_SEMANTIC_CONFIGURATION_NONCLAIMS
