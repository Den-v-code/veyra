...
`src/core/theorem_registry.py` now includes:
...
Implemented in `src/core/algebra_analysis_cards.py`:
...
Tests in `tests/test_algebra_analysis_cards.py` verify unique/identity/blocked equations, polynomial identity/evaluation, stable/blocked continuity, drift stability for square symmetric quotients, and midpoint area additivity.
