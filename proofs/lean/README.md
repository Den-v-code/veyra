...
| `VeyraRealizationTransport.lean` | abstract inverse-image partition identity/composition, label-partition representation/pullback and raw off-diagonal distinction bridge, bottom/refinement preservation, and conditional cost nonincrease | `INTERNAL_RESEARCH_CANDIDATE` | none; relation-level helpers plus an explicit `SamePartitionLabels` normalization premise; exhaustive Python small-map rows pressure that premise and production `distinction_set`, but no Python implementation equivalence, greatest-admitted R16 descent, residual-chain correspondence, P1-A vertical law, category/functor/naturality, or cross-doctrine theorem |
