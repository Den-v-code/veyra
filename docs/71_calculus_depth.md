...
Implemented in `src/core/calculus_depth.py`:
...
- transcendental functions are now only finite formal seeds in `docs/76_transcendental_limit_seed.md`;
- no measure theory;
...
```bash
PYTHONPATH=. python3 -m pytest -q tests/test_calculus_depth.py tests/test_certify.py tests/test_essence_core.py
```
