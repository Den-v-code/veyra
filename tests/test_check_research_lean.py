...
def test_canonical_manifest_binds_exact_inventory_and_toolchain() -> None:
    """The checked candidate has one exact 53+10/94 identity."""
    ...
    assert len(manifest.declarations) == 94
    assert len(manifest.headlines) == 48
    assert len(manifest.headline_claims) == 48
    assert len(manifest.axiom_closure) == 94
...
def test_generated_audit_covers_every_declaration_exactly() -> None:
    ...
    assert source.count("#check Veyra.") == 94
    assert source.count("#print axioms Veyra.") == 94
