...
EXPECTED_DECLARATION_COUNT = 93
EXPECTED_HEADLINE_COUNT = 47
MAX_JOBS = 16
...
def run(argv: list[str]) -> int:
    ...
    print("[2/6] Verifying 53 base + 10 research source rows", flush=True)
    ...
    with tempfile.TemporaryDirectory(prefix="research-lean-", dir=TMP_ROOT) as directory:
        ...
        print(
            f"[4/6] Fresh-compiling 63 sources in {len(layers)} layers with {args.jobs} workers",
            flush=True,
        )
        ...
        if failed or skipped:
            ...
            return 1
        print("[5/6] Checking all 93 declarations and exact axiom closure", flush=True)
    ...
    print(
        f"[done] passed={passed} failed=0 skipped=0 declarations=93 "
        f"axiom_rows=93 elapsed={elapsed:.2f}s "
