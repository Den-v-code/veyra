...
**Implementation:** `src/core/native_number.py`, `src/core/tact_similarity.py`.
...
```bash
PYTHONDONTWRITEBYTECODE=1 python3 -m pytest -q tests/test_native_resonance_number.py tests/test_tact_similarity.py tests/test_certify.py
...
Sprint A remains the base cycle-echo layer. Sprint X2 now extends it with `docs/98_native_number_theory_x2.md`: cycle divisibility rows, resonance-prime obstruction rows, and factor-lift rank comparisons.
