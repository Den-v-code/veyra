...
A compact gap is either a non-covered curriculum node or a covered node with missing theorem specs.  The compact node ledger currently has no gaps.  Deeper school-to-11 work is tracked separately by topic rows in `src/core/curriculum_topics.py` and `docs/70_curriculum_core_language_v08.md`.
...
Implemented in `src/core/curriculum_map.py`:
...
`src/core/curriculum_topics.py` adds `SchoolTopicCoverage` rows:
...
See `docs/70_curriculum_core_language_v08.md` for the full native-definition/shadow/example/counterexample table.
...
Tests in `tests/test_curriculum_map.py` verify summary counts, cross-domain edges, compact gap detection, domain coverage, Sage export rows, and the Core Language v0.8 topic contract.
